...
def import_excel_to_sqlite(excel_file_path, db_file_path, table_name='student_directory'):
    """
    ...
    try:
        ...
        df['mailing_address_1'] = df['mailing_address_1'].apply(standardize_address)
        df['mailing_address_2'] = df['mailing_address_2'].apply(standardize_address)
        # -------------------------------------
...
def process_and_export_households(db_file_path, output_excel_path):
    ...
    try:
        ...
        df['full_guardian_name'] = df['guardian_first_name'].str.strip() + ' ' + df['guardian_last_name'].str.strip()

        # Create a display name for kids: First Name (Grade)
        df['kid_display'] = df['student_first_name'].str.strip() + ' (' + df['grade'].astype(str).str.strip() + ')'
        ...
        grouped_df = df.groupby('household_id').agg({
        ...
        desired_order = ['Last Name', 'Kids Names', 'Adult Names', 'Address', 'Address Line 2', 'Phone', 'Household ID']
